% Description: compares the upper bound on subgraph centrality computed by 
% Gauss--Radau quadrature applied to the symmetric bipartite network 
% representation in dependence of the number of Lanczos iterations to the 
% 'exact' quantity computed with expm for the temporal E-Mail EU network 
% and plots the approximation error used in [1, Fig. 7].
% 
...
SC=diag(expm(beta*A_bipartite));

% specify range of iterations
maxit=2:10;
upper_gauss_radau_subgraph=zeros(2*nL,length(maxit));
...
% loop over range of iterations to approximate SC with Gauss
% quadrature rules
...
%% error plot
figure(2)
semilogy(maxit,max(abs(SC-upper_gauss_radau_subgraph)))
