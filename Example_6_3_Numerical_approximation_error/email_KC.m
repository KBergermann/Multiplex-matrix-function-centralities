% Description: compares the approximation of Katz centrality obtained with
% Krylov subspace methods to the 'exact' quantity computed with the
% backslash for the temporal E-Mail EU network and plots the approximation
...
KC_broadcast=((eye(size(A))-alpha*A)\eye(size(A)))*ones(nL,1);
KC_receive=((eye(size(A))-alpha*A')\eye(size(A)))*ones(nL,1);

% specify range of iterations
maxit=2:10;
KC_funm_kryl_broadcast=zeros(nL,length(maxit));
KC_funm_kryl_receive=zeros(nL,length(maxit));

% loop over range of iterations to approximate KC with Krylov subspace
% methods
for i=1:length(maxit)
    ...
    [KC_funm_kryl_broadcast(:,i),out1]=funm_kryl(A,ones(nL,1),param);
    [KC_funm_kryl_receive(:,i),out2]=funm_kryl(A',ones(nL,1),param);
    toc
end

%% error plot
figure(2)
semilogy(maxit,max(abs(KC_broadcast-KC_funm_kryl_broadcast)))
hold on
semilogy(maxit,max(abs(KC_receive-KC_funm_kryl_receive)))
hold off
