% Description: compares Gauss quadrature bounds on subgraph centrality to 
% the 'exact' quantity computed with expm for the weighted Scotland Yard
% network and plots the approximation errors. The upper Gauss--Radau bound 
% is displayed in [1, Fig. 7].
% 
...
%% error plot
figure(2)
semilogy(maxit,max(abs(SC-lower_gauss_subgraph)))
hold on
semilogy(maxit,max(abs(SC-lower_gauss_radau_subgraph)))
semilogy(maxit,max(abs(SC-upper_gauss_radau_subgraph)))
semilogy(maxit,max(abs(SC-upper_gauss_lobatto_subgraph)))
hold off
