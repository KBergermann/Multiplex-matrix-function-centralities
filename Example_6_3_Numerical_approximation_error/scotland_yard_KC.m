...
% backslash for the weighted Scotland Yard network and plots the
...
KC=(eye(size(A))-alpha*A)\eye(size(A))*ones(nL,1);

% specify range of iterations
maxit=2:10;
KC_funm_kryl=zeros(nL,length(maxit));
...
for j=1:length(maxit)
    ...
    [KC_funm_kryl(:,j),out] = funm_kryl(A,ones(nL,1),param);
    toc
end

%% error plot
figure(2)
semilogy(maxit,max(abs(KC-KC_funm_kryl)))
