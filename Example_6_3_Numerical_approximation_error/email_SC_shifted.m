% Description: compares the approximation of subgraph centrality obtained 
% with the shifted Arnoldi approach [1, Sec. 5.2.2] to the 'exact' quantity
% computed with expm for the temporal E-Mail EU network and plots the
% approximation error used in [1, Fig. 7].
% 
...
maxit=2:10;
SC_shifted=zeros(nL,length(maxit));
...
for j=1:length(maxit)
    ...
    [SC_part_two,~] = funm_kryl(A,ones(nL,1),param);
    toc

    %% SC part one: e_i^T f(A) [e_i+1]
    SC_part_one=zeros(nL,1);
    ...
    for i=1:nL
        ...
        [SC_part_one_temp,out2] = funm_kryl(A,ei+shift_vector,param);
        SC_part_one(i)=SC_part_one_temp(i);
    end
    toc

    % total approximation as described in [1, Sec. 5.2.2]
    SC_shifted(:,j)=SC_part_one-SC_part_two;
end

%% error plot
figure(2)
semilogy(maxit,max(abs(SC-SC_shifted)))
